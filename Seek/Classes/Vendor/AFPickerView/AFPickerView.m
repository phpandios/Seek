...
- (id)initWithFrame:(CGRect)frame backgroundImage:(NSString *)backgroundImage shadowImage:(NSString *)shadowImage glassImage:(NSString *)glassImage title:(NSString *)title {
    ...
    if (self)
    {
        ...
        if (shadowImage) {
            [self insertSubview:[self shadowView:shadowImage] atIndex:0];
        }
        if (glassImage) {
            [self insertSubview:[self glassView:glassImage] atIndex:0];
        }
        if (backgroundImage) {
            [self insertSubview:[self backgroundView:backgroundImage] atIndex:0];
...
- (UIImageView *)glassView:(NSString *)imageURL {
    ...
    UIImageView *glassImageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, self.frame.size.width, glassImage.size.height)];
